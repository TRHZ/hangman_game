...
const Hangman = (props: HangmanProps) => {
  const { category, gameStarted, onGameEnd, onGameStart, word } = props;
  ...
  return (
    <div className="strike">
      {!gameStarted && <button onClick={onGameStart}>Start Game</button>}
      {gameStarted && (
        <>
          ...
          <input
            ...
            disabled={gameOver || displayWord.join("") === selectedWord}
          />
          <button
            onClick={() => {
              setGameOver(true);
